...
## Session flow
1. Maya opens **Sysco Shop**, hands the login + any MFA to Dina (human-confirm gate), then loads her **order guide / past orders**.
2. For each of Dina's ~30 watch SKUs, Maya reads the current unit/case price and notes it; flags any item whose price moved vs. the figure Dina supplied from last week ("oil up 4.1%, $X→$Y").
3. Maya repeats on **US Foods MOXē** (separate login/MFA handoff) — same watch list, records each price.
4. Maya opens **Restaurant Depot** (member login) and reads warehouse prices for the haul-it-yourself staples (flour, cheese, canned tomato, gloves).
5. Maya opens **WebstaurantStore** (no login needed to read) for disposables/smallwares prices.
6. Maya builds a **side-by-side compare table**: per SKU, the price at each channel + cheapest-this-week + the week-over-week delta, and surfaces the 3–5 items that moved most.
7. Maya proposes a **split order**: which SKUs to buy where to minimize total cost (respecting that Restaurant Depot is a pickup, not delivery).
8. **Human-confirm before any cart/checkout.** With Dina's OK per channel, Maya adds items to each portal's cart and stops at the review-order screen.
9. Maya **does not submit**; Dina reviews quantities/substitutions and clicks place-order herself on each portal (irreversible-action gate).
10. Maya exports the compare table + the day's price snapshot so next week's deltas have a baseline.
...
- **Irreversible action = placing an order.** Food orders cost real money and may be non-cancelable past a cutoff. Maya must **stage carts and stop at review**; Dina submits. Same for membership/payment changes.
...
- **Setup:** Use **only throwaway/sandbox or your own test accounts**. Do not enter a real restaurant's Sysco/US Foods contract credentials, real payment methods, or real PII. WebstaurantStore prices are largely public — safe to demo read-only without login. For the gated portals, demo the *handoff* (Maya pauses for login) rather than supplying live creds. Treat all prices seen as confidential.
...
- **Session script (~8 beats):**
  1. *"Start with WebstaurantStore — pull the price on these 5 disposables."* Watch Maya navigate the public catalog and read prices without a login.
  2. *"Now go to Sysco Shop and log in."* Watch Maya **stop at the login wall and hand off** — confirm it doesn't try to guess credentials.
